...
import { ANIMATION_CONSTANTS, USER_CONSTANTS, Z_INDEX_CONSTANTS, BUTTON_CONSTANTS } from '$lib/globals'
  ...
  <div class="flex flex-wrap gap-2 justify-center user-buttons-wrapper">
    {#each users.filter((user: User) => user.username !== '') as user, index}
      ...
      >
          ...
          const randomNumber = Math.floor(Math.random() * (USER_CONSTANTS.RANDOM_ANSWER_MAX - USER_CONSTANTS.RANDOM_ANSWER_MIN + 1)) + USER_CONSTANTS.RANDOM_ANSWER_MIN
